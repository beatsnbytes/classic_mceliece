...
void syndrome_kernel2_2(unsigned char *pk_in, unsigned char *e_in, unsigned char *s_out)
{
	...
	unsigned char local_s[SYND_BYTES/2];
	...
	for(int i=0;i<MAT_ROWS/2;i++){
		for(int j=0;j<PK_ROW_BYTES;j++){
			...
			local_pk[i][j] = *(pk_in+(PK_ROW_BYTES*MAT_ROWS/2)+i*PK_ROW_BYTES+j);
		}
	}

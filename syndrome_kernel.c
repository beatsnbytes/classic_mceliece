...
void syndrome_kernel(unsigned char *pk_in, unsigned char *e_in, unsigned char *s_out)
{
	...
	unsigned char b, row[MAT_COLS]; //cols=1664

	unsigned char local_pk[MAT_ROWS][PK_ROW_BYTES];
	unsigned char local_s[SYND_BYTES]; //synd_bytes=208
	unsigned char local_e[MAT_COLS];

	#pragma HLS ARRAY_PARTITION variable=row cyclic factor=208
	#pragma HLS ARRAY_PARTITION variable=local_e cyclic factor=208
	#pragma HLS ARRAY_PARTITION variable=local_s cyclic factor=2
	#pragma HLS ARRAY_PARTITION variable=local_pk cyclic factor=208 dim=2

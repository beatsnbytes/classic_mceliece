...
void syndrome_kernel7_7(unsigned char *pk_in, unsigned char *e_in, unsigned char *s_out)
{
	...
	for(int i=0;i<MAT_ROWS/7;i++){
		for(int j=0;j<PK_ROW_BYTES;j++){
			...
			local_pk[i][j] = *(pk_in+(6*PK_ROW_BYTES*MAT_ROWS/7)+i*PK_ROW_BYTES+j);
		}
	}
...
//		#pragma HLS unroll factor=17
	...
	LOOP_WRITE_TO_BRAM_R:for (unsigned int i=6*SYND_BYTES/7;i<7*SYND_BYTES/7;i++){
		#pragma HLS PIPELINE ΙΙ=1
		#pragma HLS unroll factor=2
		*(s_out+i) = local_s[i-6*SYND_BYTES/7];
	}

...
void syndrome_kernel_3(unsigned char *pk_in, unsigned char *e_in, unsigned char *s_out)
{
...
//		#pragma HLS unroll factor=2
		...
		row[(i/8)+2*SYND_BYTES/4] |= 1 << (i%(uint)8);
		...
		local_s[(i/8)+2*SYND_BYTES/4] |= (b << (i%(uint)8));

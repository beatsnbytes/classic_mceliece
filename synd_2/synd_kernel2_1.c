...
static inline gf gf_sq_kernel2_1(gf in)
{
...
gf gf_inv_kernel2_1(gf in)
{
	...
	out = gf_sq_kernel2_1(out);
	tmp_11 = gf_mul_kernel2_1(out, in); // 11

	out = gf_sq_kernel2_1(tmp_11);
	out = gf_sq_kernel2_1(out);
	tmp_1111 = gf_mul_kernel2_1(out, tmp_11); // 1111

	out = gf_sq_kernel2_1(tmp_1111);

	out = gf_sq_kernel2_1(out);
	out = gf_sq_kernel2_1(out);
	out = gf_sq_kernel2_1(out);

	out = gf_mul_kernel2_1(out, tmp_1111); // 11111111

	out = gf_sq_kernel2_1(out);
	out = gf_sq_kernel2_1(out);
	out = gf_mul_kernel2_1(out, tmp_11); // 1111111111

	out = gf_sq_kernel2_1(out);
	out = gf_mul_kernel2_1(out, in); // 11111111111

	return gf_sq_kernel2_1(out); // 111111111110
}
...
void synd_kernel2_1(gf *out_out, gf *f_in, gf *L_in, unsigned char *r_in)
{
	...
	#pragma HLS ARRAY_PARTITION variable=local_out cyclic factor=64 //64

...
gf gf_add_kernel2_1(gf in0, gf in1)
{
	return in0 ^ in1;
}

gf gf_mul_kernel2_1(gf in0, gf in1)
{
	int i;
...
/* return: (in^2)^2 */
static inline gf gf_sq2_kernel2_1(gf in)
{
	int i;
...
/* return: (in^2)*m */
static inline gf gf_sqmul_kernel2_1(gf in, gf m)
{
	int i;
...
/* input: field element in, m */
/* return: ((in^2)^2)*m */
static inline gf gf_sq2mul_kernel2_1(gf in, gf m)
{
	int i;
...
gf gf_frac_kernel2_1(gf den, gf num)
{
	gf tmp_11;
	gf tmp_1111;
	gf out;

	tmp_11 = gf_sqmul_kernel2_1(den, den); // ^11
	tmp_1111 = gf_sq2mul_kernel2_1(tmp_11, tmp_11); // ^1111
	out = gf_sq2_kernel2_1(tmp_1111);
	out = gf_sq2mul_kernel2_1(out, tmp_1111); // ^11111111
	out = gf_sq2_kernel2_1(out);
	out = gf_sq2mul_kernel2_1(out, tmp_1111); // ^111111111111

	return gf_sqmul_kernel2_1(out, num); // ^1111111111110 = ^-1
}


gf gf_inv_kernel2_1(gf den)
{
	return gf_frac_kernel2_1(den, ((gf) 1));
}



gf eval_inner2_1(gf *f, gf a)
{
        int i;
        gf r;
...
void synd_kernel2_1(gf *out_out, gf *f_in, gf *L_in, unsigned char *r_in)
{
...
//		#pragma HLS unroll factor=2
			...
			if(i==0){
				local_out[j] = gf_mul_kernel2_1(e_inv, c);
			}else{
				local_out[j] ^= gf_mul_kernel2_1(e_inv, c);

			}
			e_inv = gf_mul_kernel2_1(e_inv, local_L[i]);

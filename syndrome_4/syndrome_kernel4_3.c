...
void syndrome_kernel4_3(unsigned char *pk_in, unsigned char *e_in, unsigned char *s_out)
{
	#pragma HLS INTERFACE m_axi     port=pk_in  offset=slave bundle=gmem6
	#pragma HLS INTERFACE m_axi     port=e_in   offset=slave bundle=gmem7
	#pragma HLS INTERFACE m_axi     port=s_out  offset=slave bundle=gmem8
	...
	#pragma HLS ARRAY_PARTITION variable=row cyclic factor=96
	#pragma HLS ARRAY_PARTITION variable=local_e cyclic factor=96
	#pragma HLS ARRAY_PARTITION variable=local_s cyclic factor=2
	#pragma HLS ARRAY_PARTITION variable=local_pk cyclic factor=96 dim=2
	...
	for(int i=0;i<MAT_ROWS/4;i++){
		for(int j=0;j<PK_ROW_BYTES;j++){
			...
			local_pk[i][j] = *(pk_in+(2*PK_ROW_BYTES*MAT_ROWS/4)+i*PK_ROW_BYTES+j);
		}
	}
	...
	LOOP_MAIN:
	for (unsigned int i = 0; i < PK_NROWS/4; i++)
	{
//	#pragma HLS PIPELINE

		LOOP_INIT_ROW:for (unsigned int i = 0; i < MAT_COLS - PK_ROW_BYTES; i++){
			#pragma HLS PIPELINE ΙΙ=1
			#pragma HLS unroll factor=156
			row[i] = 0;
		}
...
//			#pragma HLS DEPENDENCE variable=row inter false
	...
	LOOP_WRITE_TO_BRAM_R:for (unsigned int i=2*SYND_BYTES/4;i<3*SYND_BYTES/4;i++){
		#pragma HLS PIPELINE ΙΙ=1
		#pragma HLS unroll factor=2
		*(s_out+i) = local_s[i-2*SYND_BYTES/4];
	}

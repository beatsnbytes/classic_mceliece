...
gf gf_mul_kernel29_28(gf in0, gf in1)
{
	int i;
...
/* return: (in^2)^2 */
static inline gf gf_sq2_kernel29_28(gf in)
{
	int i;
...
/* return: (in^2)*m */
static inline gf gf_sqmul_kernel29_28(gf in, gf m)
{
	int i;
...
/* input: field element in, m */
/* return: ((in^2)^2)*m */
static inline gf gf_sq2mul_kernel29_28(gf in, gf m)
{
	int i;
...
gf gf_frac_kernel29_28(gf den, gf num)
{
	gf tmp_11;
	gf tmp_1111;
	gf out;

	tmp_11 = gf_sqmul_kernel29_28(den, den); // ^11
	tmp_1111 = gf_sq2mul_kernel29_28(tmp_11, tmp_11); // ^1111
	out = gf_sq2_kernel29_28(tmp_1111);
	out = gf_sq2mul_kernel29_28(out, tmp_1111); // ^11111111
	out = gf_sq2_kernel29_28(out);
	out = gf_sq2mul_kernel29_28(out, tmp_1111); // ^111111111111

	return gf_sqmul_kernel29_28(out, num); // ^1111111111110 = ^-1
}


gf gf_inv_kernel29_28(gf den)
{
	return gf_frac_kernel29_28(den, ((gf) 1));
}



gf eval_inner29_28(gf *f, gf a)
{
        int i;
        gf r;
...
void synd_kernel29_28(gf *out_out, gf *f_in, gf *L_in, unsigned char *r_in)
{
	...
	LOOP_EVAL:
	for(uint i=27*SYS_N/29; i <28*SYS_N/29; i++){
//	#pragma HLS PIPELINE
//	#pragma HLS unroll factor=2
		e_mat[i] = eval_inner29_28(local_f, local_L[i]);
	}



	LOOP_MAIN_OUTER:
	for (uint i = 27*SYS_N/29; i < 28*SYS_N/29; i++) //12
	{
//	#pragma HLS pipeline
...
//		#pragma HLS unroll factor=2
			...
			e_inv = gf_mul_kernel29_28(e_inv, local_L[i]);

...
void synd_kernel29_11(gf *out_out, gf *f_in, gf *L_in, unsigned char *r_in)
{
	...
	LOOP_LOAD_FROM_BRAM_L:for (uint i=10*SYS_N/29;i<11*SYS_N/29;i++){
	#pragma HLS PIPELINE II=1
	#pragma HLS unroll factor=4
		local_L[i] = *(L_in+i);
	}

	LOOP_LOAD_FROM_BRAM_R:for (uint i=10*MAT_COLS/29;i<11*MAT_COLS/29;i++){
	#pragma HLS PIPELINE II=1
//	#pragma HLS unroll factor=2
		local_r[i] = *(r_in+i);
	}
	...
	LOOP_EVAL:
	for(uint i=10*SYS_N/29; i <11*SYS_N/29; i++){
//	#pragma HLS PIPELINE
//	#pragma HLS unroll factor=2
	...
	LOOP_MAIN_OUTER:
	for (uint i = 10*SYS_N/29; i < 11*SYS_N/29; i++) //12
	{
//	#pragma HLS pipeline

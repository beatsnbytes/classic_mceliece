...
void synd_kernel8_2(gf *out_out, gf *f_in, gf *L_in, unsigned char *r_in)
{
	...
	LOOP_LOAD_FROM_BRAM_L:for (uint i=SYS_N/8;i<2*SYS_N/8;i++){
	#pragma HLS PIPELINE II=1
	#pragma HLS unroll factor=4
		local_L[i] = *(L_in+i);
	}

	LOOP_LOAD_FROM_BRAM_R:for (uint i=MAT_COLS/8;i<2*MAT_COLS/8;i++){
	#pragma HLS PIPELINE II=1
//	#pragma HLS unroll factor=2
		local_r[i] = *(r_in+i);
	}
	...
	LOOP_EVAL:
	for(uint i=SYS_N/8; i <2*SYS_N/8; i++){
//	#pragma HLS PIPELINE
//	#pragma HLS unroll factor=2
	...
	LOOP_MAIN_OUTER:
	for (uint i = SYS_N/8; i < 2*SYS_N/8; i++) //12
	{
//	#pragma HLS pipeline

...
//
	...
	#pragma HLS ARRAY_PARTITION variable=localMat cyclic factor=172 dim=2
	#pragma HLS ARRAY_PARTITION variable=tmpRow cyclic factor=172 //76
	...
	OUTER_LOOP_2:for (j = 0; j < 8; j++)
		{
			...
			{
				#pragma HLS dependence variable=tmpRow inter false
				#pragma HLS LOOP_TRIPCOUNT min=1 max=1663
